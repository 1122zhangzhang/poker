...
public class Poker {
    ...
    public static String poker(String black, String white) {
        ...
        //将字符串转数组
        char[] blackChars = black.toCharArray();
        char[] whiteChars = white.toCharArray();

        int[] blackNum = new int[5];
        int[] blackColors = new int[5];
        int[] whiteNum = new int[5];
        int[] whiteColors = new int[5];
        //把两个人的牌花色和数字分开
        //输入字符串带空格
        for (int i = 0, j = 0; i < blackChars.length; i += 3, j++) {
            blackNum[j] = (char) turn2Int(blackChars[i]);
        }
        for (int i = 0, j = 0; i < whiteChars.length; i += 3, j++) {
            whiteNum[j] = (char) turn2Int(whiteChars[i]);
        }

        for (int i = 1, j = 0; i < blackChars.length; i += 3, j++) {
            blackColors[j] = (char) turn2Int(blackChars[i]);
        }
        for (int i = 1, j = 0; i < blackChars.length; i += 3, j++) {
            whiteColors[j] = (char) turn2Int(whiteChars[i]);
        }
        ...
        isStraight(whiteNum);
        dependNumType(whiteNum);
        int whiteType = pokerType();


        //比较牌型
        if (blackType == whiteType) {
            return returnWhoWin(blackNum, whiteNum) == 0 ? "Tie" :
                    (returnWhoWin(blackNum, whiteNum) == 1 ? "Black wins" : "White wins");
